    ...
    try {
      ...
      if (chosenSlot) {
        ...
        if (matchedSlot) {
          ...
          const unixSec = extractUnixSec(matchedSlot.time_text);
          if (unixSec) {
            finalTimestamp = new Date(unixSec * 1000).toISOString();
          }
        } else {
        ...
        const unixSec = extractUnixSec(customTime);
        if (unixSec) {
          finalTimestamp = new Date(unixSec * 1000).toISOString();
        }
      } else {
      ...
      // Format time display for Discord native rendering
      let timeDisplay = finalTimeText;
      if (!timeDisplay.startsWith('<t:')) {
        const unixSec = extractUnixSec(finalTimestamp || finalTimeText);
        if (unixSec) {
