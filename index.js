...
discordClient.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    try {
      ...
      // Handle: START GAME (Host only, or any lobby participant when 2+ players are ready)
      if (customId === 'async_start') {
        if (players.includes(userId) && players.length >= 2) {
          ...
          await message.edit({ 
            ...
            embeds: [embed],
            components: [] // Strip off the components once started
          }).catch(() => {});
          return;
        }
      }
      ...
      // Handle: PING PLAYERS (Host only, has a 45 min cooldown)
